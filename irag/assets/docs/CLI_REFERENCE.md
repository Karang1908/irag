...
Regenerate `CLAUDE.md` **and** `AGENTS.md` (identical content — the
latter is the cross-tool convention read by Codex, Antigravity, and
other non-Claude agents) at the repo root from current page bodies:
"generated by irag" header, overview pages first, per-page staleness /
contradiction warnings. Both are ignored by irag's own ingestion, so
the export never feeds its own queue.
...
pages then folder rollups) → lint (fact-check) → export (regenerate
CLAUDE.md + AGENTS.md). The generated context file instructs agents to
run this after finishing edits, so page versions advance with every
...
changed.
Also regenerates `CLAUDE.md`, which now ends with an instruction block
teaching agents to use `irag map`/`impact`/`search`/`why` instead of
exploring, and to log with `irag learn`/`irag record-decision`.
